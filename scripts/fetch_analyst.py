...
#
# 한국 종목은 야후에서 KOSPI=.KS, KOSDAQ=.KQ 접미를 붙여야 조회된다.
# 소형주는 컨센서스·PER이 전부 없는 것이 정상이다(빈 값은 None으로 저장한다).
#
# 사용법:
#     python scripts/fetch_analyst.py 005930 211050
...
def yahoo_symbol(ticker: str, market: str = "") -> str:
    # 시장에 맞는 야후 접미를 붙인다. 시장을 모르면 코스피로 가정한다.
    return f"{str(ticker).strip()}{SUFFIX.get(str(market).strip(), '.KS')}"
