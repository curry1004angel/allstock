...
from canslim_items import (industry_stats, judge_a, judge_c, judge_i_kr, judge_l,
...
BENCHMARK_INDEX = "KS11"      # 조정 구간 판정 기준 지수는 코스피다.
...
def main():
    ...
    dd = _index_drawdown(b.indices.get(BENCHMARK_INDEX))
    if dd:
    ...
    for ticker in b.results.index:
        try:
            ...
            # 종목 하나의 예외로 전체 실행이 죽으면 일일 워크플로가 결과 자체를 못
            # 만든다. 그렇다고 통째로 삼키면 어떤 종목이 왜 빠졌는지 알 수 없으므로
            # 티커와 예외를 반드시 출력한다.
