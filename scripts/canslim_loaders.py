...
# 따로 정하다 종목의 38%가 어긋났던 것과 같은 사고다).
MIN_BASE_ROWS = 60
...
class Bundle:
    ...
    # 미국 기관 보유 이력. 한국에서는 늘 비어 있지만 canslim_items.py를 두 레포에서
    # 동일하게 유지하려면 judge_i_us가 읽는 필드가 양쪽 Bundle에 다 있어야 한다.
...
def load_all(data_dir=Path("data")):
    ...
    # 직전의 정상 결과를 덮어쓴다. 나머지 입력과 달리 여기서는 크게 실패한다.
    ...
    # 한국 티커는 6자리 숫자 문자열이다. dtype=str이 없으면 000020이 정수 20으로
    # 파싱되어 다른 파일과의 조인이 전부 깨진다.
    ...
    flows = _read_parquet(data_dir / "flows/net_purchases.parquet",
                          ["ticker", "asof", "window", "investor", "amount"])
    ...
    return Bundle(
        ...
        prices=prices, shares=shares, analyst=analyst, flows=flows, indices=indices,
    )
